...
/* Apply uniforms from a Lua table of floats
   usage: gfx.apply_uniforms(slot, {f1, f2, f3, ...}) */
static int l_apply_uniforms(lua_State *L) {
    ...
    sg_apply_uniforms(slot, &(sg_range){ data, len * sizeof(float) });
    free(data);
    return 0;
}

/* Load shader from files (generated by sokol-shdc -f bare)
   usage: gfx.load_shader_bytecode(vs_path, fs_path, [ub_size])
   ub_size: optional uniform block size in bytes for slot 0 */
static int l_load_shader_bytecode(lua_State *L) {
    ...
    int ub_size = (int)luaL_optinteger(L, 3, 0);
    ...
    /* Set uniform block size if specified */
    if (ub_size > 0) {
        desc.uniform_blocks[0].size = ub_size;
